...
// No token management needed on the frontend — cookie is handled by the browser.
// Keep setToken/getToken as no-ops so existing call sites don't break.
...
api.interceptors.response.use(
  ...
  (err) => {
    if (err.response?.status === 401) {
      window.dispatchEvent(new Event('axiom:logout'))
    }
...
export const cvApi = {
  ...
  uploadCV:         (file: File) => {
    ...
    fd.append('file', file)
    return api.post('/cv/upload-cv', fd, { headers: { 'Content-Type': 'multipart/form-data' } }).then(r => r.data)
  },
}

export const exportApi = {
  downloadPDF:       (cvId: string): Promise<Blob>                   => api.get(`/export/pdf/${cvId}`, { responseType: 'blob' }).then(r => r.data),
  downloadPublicPDF: (username: string, slug: string): Promise<Blob> => api.get(`/export/public-pdf/${username}/${slug}`, { responseType: 'blob' }).then(r => r.data),
  previewPDF:        (data: object): Promise<Blob>                   => api.post('/export/pdf-preview', data, { responseType: 'blob' }).then(r => r.data),
}

export const publicApi = {
  getCV:     (username: string, slug: string) => api.get(`/public/cv/${username}/${slug}`).then(r => r.data),
  getProfile:(username: string)               => api.get(`/public/profile/${username}`).then(r => r.data),
  getFeed:   (skip = 0, limit = 12)           => api.get(`/public/feed?skip=${skip}&limit=${limit}`).then(r => r.data),
}
