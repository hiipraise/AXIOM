  ...
  return (
    <div className="min-h-screen bg-ash flex">
      ...
      <aside
        ...
        style={{ top: bannerH, bottom: 0, transition: 'top 0.28s cubic-bezier(0.4,0,0.2,1)' }}
      >
        <SidebarContent />
      </aside>
      ...
      >
        <span className="font-display text-sm font-bold text-ink">AXIOM Admin</span>
        <button onClick={() => setOpen(true)} className="p-2 text-ink-muted hover:text-ink"><Menu size={18} /></button>
      </div>
      ...
      <AnimatePresence>
        {open && (
          <>
            ...
            <motion.aside className="md:hidden fixed top-0 left-0 h-full w-64 bg-white z-50 flex flex-col shadow-xl"
              initial={{ x: '-100%' }} animate={{ x: 0 }} exit={{ x: '-100%' }}
              transition={{ type: 'spring', stiffness: 300, damping: 30 }}>
              <button onClick={() => setOpen(false)} className="absolute top-3 right-3 p-1.5 text-ink-muted hover:text-ink"><X size={16} /></button>
      ...
      <main
        ...
        style={{ paddingTop: bannerH + 48, transition: 'padding-top 0.28s cubic-bezier(0.4,0,0.2,1)' }}
      >
