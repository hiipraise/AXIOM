    ...
    <div className="space-y-5 animate-fade-in">
      ...
      {items.map((item, i) => (
        <Card key={i} className="space-y-3">
          ...
          <Field label="Description"><Textarea name={`award-desc-${i}`} showWordCount value={stripMarkdown(item.description)} onChange={(e) => update(i, { description: stripMarkdown(e.target.value) })} rows={2} /></Field>
        </Card>
      ))}
    </div>
  )
    ...
    <div className="space-y-5 animate-fade-in">
      ...
      {items.map((item, i) => (
        <Card key={i} className="space-y-3">
          ...
          <Field label="Description"><Textarea name={`vol-desc-${i}`} showWordCount value={stripMarkdown(item.description)} onChange={(e) => update(i, { description: stripMarkdown(e.target.value) })} rows={2} /></Field>
        </Card>
      ))}
    </div>
  )
