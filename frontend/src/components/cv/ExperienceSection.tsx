...
export default function ExperienceSection({
  ...
  const updateAchievement = (i: number, ai: number, val: string) =>
    update(i, {
      achievements: items[i].achievements.map((a, idx) =>
        idx === ai ? val : a,
      ),
    });
    ...
    <div className="space-y-5 animate-fade-in">
      <SectionHeader
        ...
        action={
          <button
            onClick={add}
            className="flex items-center gap-1.5 px-3 py-1.5 text-xs bg-ink text-white rounded-lg hover:bg-ink-light transition-colors"
          >
      ...
      {items.length === 0 && (
        <div className="text-center py-10 border-2 border-dashed border-ash-border rounded-xl">
          ...
          <button onClick={add} className="mt-3 text-xs text-ink underline">
            Add your first role
          </button>
        </div>
      )}
