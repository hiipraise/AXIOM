...
import { axiomJobsApi, jobsApi } from "../../api";
import { AxiomJob, JobResult } from "../../types";
...
function normalizeExternalJob(job: JobResult): TeaserJob {
  return {
    id: job.id,
    title: job.title,
    company: job.company,
...
function JobFeedSkeleton() {
  return (
    <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-12" aria-label="Loading featured jobs">
      {Array.from({ length: 3 }).map((_, i) => (
        <div key={i} className="rounded-2xl border border-ash-border bg-white p-5 animate-pulse">
...
export default function JobsTeaserSection() {
  ...
  const { data: jobs = [], isLoading } = useQuery({
    ...
    queryFn: async () => {
      ...
      if (axiomJobs.length) return axiomJobs.slice(0, 3).map(normalizeAxiomJob);

      const external = await jobsApi.search({ per_page: 3 });
      return (external.items || []).slice(0, 3).map(normalizeExternalJob);
    },
    staleTime: 1000 * 60 * 5,
  });
      ...
      <div className="max-w-5xl mx-auto" ref={ref}>
        ...
        >
          ...
          <p className="text-ink-muted text-sm max-w-md mx-auto leading-relaxed">
            AXIOM is more than a CV generator now. Browse roles, apply on-platform,
            track progress, and prepare for the interview loop.
          </p>
        </motion.div>
              ...
              >
                ...
                >
                  <div className="mb-4 flex items-center justify-between gap-3">
                    <span className={`badge ${job.source === "axiom" ? "bg-emerald-50 text-emerald-700 border border-emerald-200" : "bg-ash-dark text-ink-muted"}`}>
                      {job.source === "axiom" ? "AXIOM" : "Live role"}
                    </span>
                    {job.remote && <span className="text-[11px] text-ink-muted">Remote</span>}
                  </div>
