...
// Flat index across all groups so every icon has a unique float offset
          ...
          <div ref={dropdownRef} className="relative">
            <button
              ...
              className={`flex items-center gap-1.5 px-4 py-2 rounded-lg text-sm transition-all ${
                featuresOpen
                  ? 'bg-ash text-ink'
                  : 'text-ink-muted hover:text-ink hover:bg-ash'
              }`}
            >
              Features
              <ChevronDown
              ...
              <div className="absolute top-full mt-2 left-1/2 -translate-x-1/2 w-[640px] bg-white border border-ash-border rounded-2xl shadow-xl p-5 grid grid-cols-3 gap-4 animate-fade-in">
                ...
                <div className="col-span-3 border-t border-ash-border pt-4 mt-1 flex items-center justify-between">
                  <p className="text-xs text-ink-muted">
                    Zero-cliché AI writing · ATS-safe PDF · Version history
                  </p>
                  <Link
                    to="/register"
      ...
      {mobileOpen && (
        <div className="md:hidden bg-white border-t border-ash-border px-5 py-4 space-y-1 animate-fade-in">
          ...
          {FEATURES.flatMap(g => g.items).map(({ icon: Icon, label }, i) => (
            <div
              key={label}
              className="flex items-center gap-2.5 px-3 py-2 rounded-lg text-sm text-ink-muted"
            >
              <motion.div
                animate={{ y: [0, -3, 0] }}
                transition={{
          ...
          <div className="border-t border-ash-border pt-3 mt-2 space-y-1">
            <a
              ...
              className="block px-3 py-2 text-sm text-ink-muted rounded-lg hover:bg-ash"
            >
              How it works
            </a>
            <Link
            ...
            <Link
              to="/login"
              onClick={() => setMobileOpen(false)}
              className="block px-3 py-2 text-sm text-ink font-medium rounded-lg hover:bg-ash"
            >
              Sign in
            </Link>
            <Link
              to="/register"
              onClick={() => setMobileOpen(false)}
              className="block px-3 py-2 bg-ink text-white text-sm font-medium rounded-lg text-center mt-2"
            >
              Get started free
            </Link>
          </div>
