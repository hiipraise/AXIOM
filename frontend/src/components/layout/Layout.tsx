  ...
  return (
    <>
      <div className="px-5 py-5 border-b border-ash-border">
        <Link to="/" onClick={onNav} className="font-display text-xl font-bold text-ink tracking-tight">AXIOM</Link>
      ...
      <nav className="flex-1 px-3 py-4 space-y-0.5 overflow-y-auto">
        {NAV.map(({ to, label, icon: Icon }) => (
          <NavLink key={to} to={to} onClick={onNav}
            className={({ isActive }) => clsx('flex items-center gap-2.5 px-3 py-2 rounded-lg text-sm transition-all',
              isActive ? 'bg-ink text-white' : 'text-ink-muted hover:bg-ash hover:text-ink')}>
            <Icon size={15} /> {label}
          </NavLink>
        ))}
        <button onClick={() => { navigate('/cv/new'); onNav?.() }}
      ...
      <AnimatePresence>
        {open && (
          <>
            ...
            <motion.aside className="md:hidden fixed top-0 left-0 h-full w-72 bg-white z-50 flex flex-col shadow-xl"
