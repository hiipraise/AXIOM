...
function SectionDrawer({
  activeSection, onSelect, onClose,
}: {
    ...
    <div className="flex bg-ash overflow-hidden"
      ...
      <div className="hidden md:flex w-44 bg-white border-r border-ash-border flex-col flex-shrink-0">
        <div className="px-3 py-3 border-b border-ash-border flex-shrink-0">
          <button onClick={handleLeaveEditor}
        ...
        <div className="flex-1 overflow-y-auto py-2">
          {SECTIONS.map(({ id: sid, label, icon: Icon }) => (
            <button key={sid} onClick={() => setActiveSection(sid)}
              className={`w-full flex items-center gap-2.5 px-3 py-2 text-xs transition-all text-left ${activeSection === sid ? "bg-ink text-white font-medium" : "text-ink-muted hover:bg-ash hover:text-ink"}`}>
              <Icon size={13} /> {label}
            </button>
          ))}
        </div>
        <div className="p-3 border-t border-ash-border space-y-1 flex-shrink-0">
          ...
          <button onClick={handleOpenRating}
            className={`w-full flex items-center gap-2 px-3 py-2 rounded-lg text-xs border transition-colors ${currentRating ? "border-amber-200 bg-amber-50 text-amber-700 hover:bg-amber-100" : "border-ash-border text-ink-muted hover:bg-ash"}`}>
        ...
        <div className="bg-white border-b border-ash-border px-3 sm:px-5 py-2.5 flex items-center gap-2 flex-shrink-0">
          ...
          <div className="flex items-center gap-1.5 ml-auto flex-shrink-0">
            ...
            <button onClick={handleSave} disabled={saving || !isDirty}
              className={`flex items-center gap-1.5 text-xs px-3 py-1.5 rounded-lg font-medium transition-colors ${isDirty ? "bg-ink text-white hover:bg-ink-light" : "bg-ash text-ink-muted cursor-not-allowed"}`}>
              <Save size={12} />
