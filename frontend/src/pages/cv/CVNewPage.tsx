...
import { Upload, MessageSquare, FileText, ChevronRight, ArrowLeft } from 'lucide-react'
...
export default function CVNewPage() {
  ...
  return (
    <div className="min-h-screen bg-ash px-6 py-8">
      <div className="max-w-2xl mx-auto">
        <button onClick={() => mode === 'choose' ? navigate('/dashboard') : setMode('choose')} className="flex items-center gap-1.5 text-xs text-ink-muted hover:text-ink mb-6 transition-colors">
          <ArrowLeft size={13} /> {mode === 'choose' ? 'Back to dashboard' : 'Back to options'}
        </button>

        <h1 className="font-display text-2xl font-bold text-ink mb-1">New CV</h1>
        <p className="text-sm text-ink-muted mb-8">Choose how you want to start</p>
          ...
          <div className="grid grid-cols-1 gap-4">
            {[
              ...
              <button key={m} onClick={() => setMode(m)}
                className="flex items-start gap-4 p-5 bg-white rounded-xl border border-ash-border hover:border-ink transition-all text-left group">
                <div className="w-9 h-9 rounded-lg bg-ash flex items-center justify-center mt-0.5 group-hover:bg-ink group-hover:text-white transition-colors">
                  <Icon size={16} />
                </div>
                ...
                <ChevronRight size={16} className="text-ink-muted mt-2 group-hover:text-ink transition-colors" />
              </button>
            ))}
          </div>
        )}
        ...
        {(mode === 'blank' || mode === 'upload') && (
          <div className="bg-white rounded-xl border border-ash-border p-6 space-y-5">
            <div>
              ...
              <div className="flex gap-2">
                {[1, 2, 3].map((n) => (
                  <button key={n} onClick={() => setPageCount(n)}
                    className={`px-4 py-2 text-xs rounded-lg border transition-colors ${pageCount === n ? 'bg-ink text-white border-ink' : 'border-ash-border text-ink-muted hover:bg-ash'}`}>
                    {n} {n === 1 ? 'page' : 'pages'}
                  </button>
                ))}
              </div>
            ...
            {mode === 'upload' && (
              <div>
                ...
                {!extractedData ? (
                  <label className={`flex flex-col items-center justify-center border-2 border-dashed border-ash-border rounded-xl p-8 cursor-pointer hover:border-ink transition-colors ${uploading ? 'opacity-50 pointer-events-none' : ''}`}>
                    <Upload size={24} className="text-ink-muted mb-2" />
                    <span className="text-sm text-ink-muted">{uploading ? 'Extracting…' : 'Click to upload PDF'}</span>
                    <input type="file" accept=".pdf" className="hidden" onChange={handleFileUpload} />
                  </label>
                ) : (
                  <div className="flex items-center gap-2 p-3 bg-emerald-50 border border-emerald-200 rounded-lg">
                    <FileText size={15} className="text-emerald-600" />
                    <span className="text-xs text-emerald-700 font-medium">CV extracted successfully — {Object.keys(extractedData.experience).length} experience entries found</span>
                  </div>
                )}
              </div>
            )}

            <button onClick={mode === 'blank' ? handleCreateBlank : handleCreateFromUpload}
        ...
        {mode === 'interview' && (
          <div className="bg-white rounded-xl border border-ash-border overflow-hidden">
            <div className="px-5 py-4 border-b border-ash-border">
              <p className="text-sm font-medium text-ink">AI Interview</p>
              <p className="text-xs text-ink-muted">Answer questions naturally — the AI will gather your career details</p>
            </div>
            <div className="h-80 overflow-y-auto p-4 space-y-3">
              {interviewMessages.length === 0 && (
                <div className="text-xs text-ink-muted italic">Start by telling me about yourself — your current role, your background, or what kind of job you're targeting.</div>
              )}
              {interviewMessages.map((msg, i) => (
                <div key={i} className={`flex ${msg.role === 'user' ? 'justify-end' : 'justify-start'}`}>
                  <div className={`max-w-[85%] px-3 py-2 rounded-xl text-xs leading-relaxed ${msg.role === 'user' ? 'bg-ink text-white' : 'bg-ash text-ink'}`}>
                    {msg.content}
                  </div>
                </div>
              ))}
            ...
            <div className="border-t border-ash-border p-3 flex gap-2">
              <input value={interviewInput} onChange={(e) => setInterviewInput(e.target.value)}
                ...
                placeholder="Your answer…" className="flex-1 px-3 py-2 text-xs border border-ash-border rounded-lg focus:outline-none focus:border-ink" />
              <button onClick={handleInterviewSend} disabled={interviewLoading || !interviewInput.trim()}
                className="px-3 py-2 bg-ink text-white text-xs rounded-lg hover:bg-ink-light disabled:opacity-50 transition-colors">Send</button>
            </div>
            {interviewMessages.length >= 4 && (
              <div className="p-4 border-t border-ash-border space-y-3">
                <input value={title} onChange={(e) => setTitle(e.target.value)}
                  placeholder="CV Title (required)" className="w-full px-3 py-2 text-xs border border-ash-border rounded-lg focus:outline-none focus:border-ink" />
                <button onClick={handleCreateFromInterview} disabled={loading || !title.trim()}
