...
export default function CreateJobPage({ edit = false }: { edit?: boolean }) {
  ...
  const { data: existingJob, isLoading: jobLoading } = useQuery({
    queryKey: ["axiom-job", id],
    queryFn: () => axiomJobsApi.get(id),
    enabled: edit && !!id,
  });
  ...
  if (edit && jobLoading) {
    return (
      <div className="mx-auto max-w-4xl px-4 py-8 animate-pulse">
        <div className="h-8 w-64 rounded bg-ash-dark mb-6" />
        <div className="card space-y-4">
          <div className="grid gap-3 md:grid-cols-2">
            {Array.from({ length: 8 }).map((_, i) => (
              <div key={i} className="h-10 rounded bg-ash-dark" />
            ))}
          </div>
          <div className="h-64 rounded bg-ash-dark" />
        </div>
      </div>
    );
  }
      ...
      <form className="card space-y-4" onSubmit={submit}>
        <div className="grid gap-3 md:grid-cols-2">
          ...
          <div>
            ...
            >
              {["entry", "mid", "senior", "executive"].map((v) => (
                <option key={v}>{v}</option>
              ))}
            </select>
          </div>
        ...
        <div>
          ...
          <input
            ...
            placeholder="React, Python, Hiring"
          />
        </div>
        <div>
