...
function Section({ title, accent, line, children }: { title: string; accent: string; line: string; children: React.ReactNode }) {
  return (
    <div className="mt-6">
      <p className="text-xs font-bold tracking-widest uppercase mb-1.5" style={{ color: accent }}>{title}</p>
      <div className="border-t mb-3" style={{ borderColor: line }} />
      {children}
    </div>
  )
}
...
export default function PublicCVPage() {
  ...
  const theme = getCVTheme(cv.theme)
  ...
  return (
    <div className="min-h-screen bg-ash">
      ...
      <div className="max-w-[700px] mx-auto my-8 bg-white shadow-sm rounded-xl overflow-hidden">
        <div className="p-12" style={{ fontFamily: theme.fontFamily, color: theme.text, background: theme.background }}>
          <h1 className="text-3xl font-bold mb-1" style={{ color: acc }}>{pi.full_name || username}</h1>
          {pi.job_title && <p className="text-sm mb-3" style={{ color: theme.secondary }}>{pi.job_title}</p>}
          <div className="text-xs flex flex-wrap gap-x-4 gap-y-1 pb-3 mb-3" style={{ color: theme.secondary, borderBottom: `2px solid ${acc}` }}>
            {[pi.email, pi.phone, pi.location, pi.linkedin, pi.github, pi.portfolio].filter(Boolean).map((c, i) => <span key={i}>{c}</span>)}
          </div>

          {cv.data.summary && <Section title="PROFILE" accent={acc} line={theme.line}><p className="text-sm leading-relaxed">{cv.data.summary}</p></Section>}

          {cv.data.experience.length > 0 && (
            <Section title="EXPERIENCE" accent={acc} line={theme.line}>
              {cv.data.experience.map((exp, i) => (
                <div key={i} className="mb-4">
                  <div className="flex justify-between items-baseline">
                    <span className="font-semibold text-sm">{exp.role} — {exp.company}</span>
                    <span className="text-xs" style={{ color: theme.secondary }}>{exp.start_date}{exp.start_date && ' – '}{exp.current ? 'Present' : exp.end_date}</span>
                  </div>
          ...
          {cv.data.education.length > 0 && (
            <Section title="EDUCATION" accent={acc} line={theme.line}>
              {cv.data.education.map((edu, i) => (
                <div key={i} className="mb-3">
                  <div className="flex justify-between items-baseline">
                    <span className="font-semibold text-sm">{edu.degree}{edu.field ? `, ${edu.field}` : ''} — {edu.institution}</span>
                    <span className="text-xs" style={{ color: theme.secondary }}>{edu.start_date}{edu.start_date && ' – '}{edu.end_date}</span>
                  </div>
                  {edu.grade && <p className="text-xs" style={{ color: theme.secondary }}>{edu.grade}</p>}
                </div>
              ))}
            </Section>
          )}

          {cv.data.skills.length > 0 && <Section title="SKILLS" accent={acc} line={theme.line}><p className="text-sm">{cv.data.skills.join(', ')}</p></Section>}

          {cv.data.certifications.length > 0 && (
            <Section title="CERTIFICATIONS" accent={acc} line={theme.line}>
              {cv.data.certifications.map((c, i) => (
                <div key={i} className="mb-2">
                  <span className="font-semibold text-sm">{c.name}</span>
                  <span className="text-sm" style={{ color: theme.secondary }}> — {c.issuer}{c.date ? ` (${c.date})` : ''}</span>
                  {c.url && <p className="text-xs mt-0.5" style={{ color: theme.secondary }}>{c.url}</p>}
                </div>
              ))}
            </Section>
          )}
          ...
          {cv.data.projects.length > 0 && (
            <Section title="PROJECTS" accent={acc} line={theme.line}>
              {cv.data.projects.map((p, i) => (
                <div key={i} className="mb-3">
                  <p className="font-semibold text-sm">{p.name}</p>
                  {p.description && <p className="text-sm">{p.description}</p>}
                  {p.technologies.length > 0 && <p className="text-xs mt-0.5" style={{ color: theme.secondary }}>Tech: {p.technologies.join(', ')}</p>}
                  {p.url && <p className="text-xs" style={{ color: theme.secondary }}>{p.url}</p>}
                </div>
              ))}
            </Section>
          )}
          ...
          {cv.data.languages.length > 0 && (
            <Section title="LANGUAGES" accent={acc} line={theme.line}>
