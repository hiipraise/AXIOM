      ...
      <div className="p-5">
        ...
        <div className="flex items-start justify-between gap-3 mb-3">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              {cv.personal_info.job_title && (
                <p className="text-xs text-ink-muted mt-0.5">{cv.personal_info.job_title}</p>
              )}
            </div>
          </div>
          <div className="flex items-center gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
            <button
              onClick={() => onShare(cv)}
              className="p-1.5 text-ink-muted hover:text-ink hover:bg-ash rounded-lg transition-colors"
              title="Share"
            >
              <Share2 size={13} />
            </button>
            <Link
              to={`/cv/${cv.owner_username}/${cv.slug}`}
              className="p-1.5 text-ink-muted hover:text-ink hover:bg-ash rounded-lg transition-colors"
              title="View CV"
            >
        ...
        {cv.skills.length > 0 && (
          <div className="flex flex-wrap gap-1.5 mb-4">
            {cv.skills.slice(0, 5).map(s => (
              <span key={s} className="px-2 py-0.5 bg-ash text-ink-muted text-[11px] rounded-md border border-ash-border">
                {s}
              </span>
            ))}
            {cv.skills.length > 5 && (
              <span className="px-2 py-0.5 text-ink-muted text-[11px] rounded-md">
                +{cv.skills.length - 5}
              </span>
            )}
          </div>
        ...
        <div className="flex items-center justify-between pt-3 border-t border-ash-border">
          <div className="flex items-center gap-1.5 text-[11px] text-ink-muted">
            <Globe size={11} />
            <Link to={`/profile/${cv.owner_username}`} className="hover:text-ink transition-colors">
              @{cv.owner_username}
            </Link>
...
function ShareModal({ cv, onClose }: { cv: FeedCV; onClose: () => void }) {
  ...
  return (
    <div className="fixed inset-0 bg-black/40 z-50 flex items-center justify-center p-4" onClick={onClose}>
      <div className="bg-white rounded-2xl border border-ash-border p-6 w-full max-w-sm shadow-xl animate-slide-up"
        ...
        <p className="text-xs text-ink-muted mb-3">
          Share <strong>{cv.personal_info.full_name || cv.owner_username}</strong>'s CV:
        </p>
        <div className="flex gap-2">
          <input
            readOnly value={url}
            className="flex-1 px-3 py-2 text-xs border border-ash-border rounded-lg bg-ash text-ink-muted font-mono"
          />
          <button
            onClick={copy}
            className={`px-3 py-2 text-xs font-medium rounded-lg transition-all ${
              copied ? 'bg-emerald-500 text-white' : 'bg-ink text-white hover:bg-ink-light'
            }`}
          >
            {copied ? 'Copied!' : 'Copy'}
          </button>
        ...
        <div className="flex gap-2 mt-3">
          <a
            href={`https://twitter.com/intent/tweet?url=${encodeURIComponent(url)}&text=${encodeURIComponent(`Check out this CV by @${cv.owner_username}`)}`}
            target="_blank" rel="noopener noreferrer"
            className="flex-1 text-center py-2 text-xs border border-ash-border rounded-lg hover:bg-ash text-ink-muted transition-colors"
          >
            Share on X
          </a>
          <a
...
export default function PublicFeedPage() {
  ...
  const filtered = (data?.cvs || []).filter(cv => {
    ...
    return (
      ...
      cv.personal_info.job_title?.toLowerCase().includes(q) ||
      cv.skills.some(s => s.toLowerCase().includes(q)) ||
      cv.owner_username.toLowerCase().includes(q)
    )
  })
  ...
  return (
    <div className="min-h-screen bg-ash">
      {/* Topbar */}
      <header className="bg-white border-b border-ash-border sticky top-0 z-30">
        <div className="max-w-6xl mx-auto px-5 h-14 flex items-center justify-between gap-4">
          ...
          <div className="flex items-center gap-3">
            ...
            <Link to="/register" className="px-3 py-1.5 bg-ink text-white text-xs font-medium rounded-lg hover:bg-ink-light transition-colors">
              Create yours →
            </Link>
          </div>
        </div>
      ...
      <div className="max-w-6xl mx-auto px-5 py-8">
        ...
        <div className="mb-8">
          ...
          <p className="text-sm text-ink-muted">
            {data?.total ? `${data.total} public CV${data.total === 1 ? '' : 's'} from the AXIOM community` : 'Browse public CVs from the community'}
          </p>
        </div>
        ...
        {isLoading ? (
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
            {Array.from({ length: 6 }).map((_, i) => (
              <div key={i} className="bg-white rounded-2xl border border-ash-border h-48 animate-pulse" />
            ))}
          </div>
        ) : filtered.length === 0 ? (
          <div className="text-center py-20 border-2 border-dashed border-ash-border rounded-2xl">
            ...
            <p className="text-sm text-ink-muted mb-1">
              {search ? `No results for "${search}"` : 'No public CVs yet'}
            </p>
            {search && (
            ...
            {!search && (
              <Link to="/register" className="text-xs text-ink underline mt-1 inline-block">
                Be the first — create yours
              </Link>
            )}
          </div>
        ) : (
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
            {filtered.map(cv => (
              <CVCard key={cv.id} cv={cv} onShare={setSharing} />
            ))}
          </div>
        )}
        ...
        {!search && data && data.total > limit && (
          <div className="flex items-center justify-center gap-3 mt-10">
            <button
              ...
              disabled={page === 0}
              className="px-4 py-2 text-xs border border-ash-border rounded-lg hover:bg-white disabled:opacity-40 transition-colors"
            >
              ← Previous
            </button>
